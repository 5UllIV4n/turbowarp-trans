...
(function () {
  ...
  class PythonTranspiler {
    ...
    _buildSidebar () {
      ...
      // Step 1: Add the CSS for the Tabs
      ...
      // Step 2: Build the HTML Elements
      ...
      // 2. Create the main view wrapper
      this.viewContainer = document.createElement('div');
      this.viewContainer.className = 'py-view-container';

      // 3. Create the three separate views
      ...
      // 4. Assemble the views into the container
      ...
      // Append the tab bar and the views to your main sidebar wrapper
    ...
    // Step 1: Define the Asset Renderer
    _renderAssets (target) {
      // Grab the stage (we always want to show the stage assets regardless of what sprite is selected)
    ...
    async _downloadZip () {
      ...
      // 2. Now proceed with your zip logic
      const zip = new JSZip();
    ...
    _attachListeners () {
      // Step 3: Add the Tab Switching Logic
      ...
      tabs.forEach(tab => {
        tab.addEventListener('click', () => {
          ...
          // Find the matching view and show it
...
`;
    ...
    _transpile (target) {
      ...
      // so `global x` inside a handler never refers to an undefined name.
